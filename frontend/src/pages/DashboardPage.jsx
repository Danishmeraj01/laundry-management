...
const DashboardPage = () => {
  ...
  const isCustomer = stats?.role === 'customer';
  const showAnalytics = stats?.topGarments || stats?.statusBreakdown;
    ...
    <div>
      ...
      <div style={{ padding: '32px' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(200px, 1fr))', gap: '20px', marginBottom: '32px' }}>
          {isCustomer ? (
            <>
              <StatCard label="My Orders" value={stats?.myOrders || 0} icon={ShoppingBag} color="#3b82f6" />
              <StatCard label="My Pending Orders" value={stats?.myPendingOrders || 0} icon={Clock} color="#f59e0b" />
              <StatCard label="My Delivered Orders" value={stats?.myDeliveredOrders || 0} icon={CheckCircle} color="#8b5cf6" />
              <StatCard label="My Active Orders" value={stats?.myActiveOrders || 0} icon={ShoppingBag} color="#10b981" />
            </>
          ) : (
            <>
              <StatCard label="Total Orders" value={stats?.totalOrders || 0} icon={ShoppingBag} color="#3b82f6" />
              <StatCard label="Total Revenue" value={`₹${stats?.totalRevenue || 0}`} icon={IndianRupee} color="#10b981" />
              <StatCard label="Pending Orders" value={stats?.pendingOrders || 0} icon={Clock} color="#f59e0b" />
              <StatCard label="Delivered" value={stats?.deliveredOrders || 0} icon={CheckCircle} color="#8b5cf6" />
            </>
          )}
        </div>
          ...
          <>
            <div style={{
              ...
              <div style={{ background: '#fff', borderRadius: '12px', padding: '24px', boxShadow: '0 1px 8px rgba(0,0,0,0.06)' }}>
                <h3 style={{ margin: '0 0 20px', fontSize: '16px', fontWeight: 600 }}>
                  {isCustomer ? 'My Top Garments' : 'Top Garments'}
                </h3>
                <ResponsiveContainer width="100%" height={220}>
            ...
            <div style={{ background: '#fff', borderRadius: '12px', padding: '24px', boxShadow: '0 1px 8px rgba(0,0,0,0.06)' }}>
              <h3 style={{ margin: '0 0 16px', fontSize: '16px', fontWeight: 600 }}>
                {isCustomer ? 'My Orders by Status' : 'Orders by Status'}
              </h3>
